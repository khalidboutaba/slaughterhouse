    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>الدفعات</h3>
                <p class="text-subtitle text-muted">إضافة دفعة جديدة</p>
            </div>
                                    ...
                                    <div class="col-md-6 col-12">
                                        <div class="form-group">
                                            ...
                                            <select class="choices form-select" name="payment_type">
                                                ...
                                                <option value="sale" {{ old('payment_type') === 'sale' ? 'selected' : '' }}>بيع</option>
                                                <option value="purchase" {{ old('payment_type') === 'purchase' ? 'selected' : '' }}>شراء</option>
                                            </select>
                                            @error('payment_type')
                                    ...
                                    <div class="col-md-6 col-12">
                                        <div class="form-group">
                                            ...
                                            <input type="date" id="payment_date" class="form-control"
                                                placeholder="تاريخ الأداء" name="payment_date" value="{{ old('payment_date') }}" required>
                                            @error('payment_date')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="col-md-6 col-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="amount" class="form-control"
                                                placeholder="المبلغ" name="amount" value="{{ old('amount') }}" required>
                                            @error('amount')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
...
function updateSaleInfo(select) {
    ...
    document.getElementById('sale_total').value = selectedOption.text.match(/\d+(\.\d+)?/g)[1];
    document.getElementById('sale_total_paid').value = selectedOption.text.match(/\d+(\.\d+)?/g)[2];
    updateRemainingAmount();
}
...
function updateAnimalInfo(select) {
    ...
    // Updated regex to match the total price even if there's no space before د.م
    var totalPriceMatches = selectedOption.match(/-\s([\d,]+\.\d{2})\s?د\.م/);

    // Updated regex to match the total_paid value, allowing optional space before د.م
    var totalPaidMatches = selectedOption.match(/دفع:\s([\d,]+\.\d{2})\s?د\.م/);

    // Set total_price if matched
    if (totalPriceMatches && totalPriceMatches[1]) {
        document.getElementById('animal_total').value = parseFloat(totalPriceMatches[1].replace(/,/g, ''));
    } else {
        console.error("السعر الإجمالي غير موجود.");
    }

    // Set total_paid if matched
    if (totalPaidMatches && totalPaidMatches[1]) {
        document.getElementById('animal_total_paid').value = parseFloat(totalPaidMatches[1].replace(/,/g, ''));
    } else {
        console.error("السعر الإجمالي غير موجود.");
    }
...
function updateRemainingAmount() {
    ...
    var remainingSale = saleTotal - saleTotalPaid;
    var remainingAnimal = animalTotal - animalTotalPaid;

    var totalRemaining = remainingSale + remainingAnimal;

    document.getElementById('remaining_amount').textContent = totalRemaining.toFixed(2) + ' د.م';
}
</script>
@endsection
